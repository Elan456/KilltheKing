...
def nearest_node_by_cansee_and_distance(point, map, desired_connection=None):
    search_size = 100
    wh = len(map.node_map) * 25
    ww = len(map.node_map[0]) * 25
    while True:
        cansee = []
            ...
            if desired_connection is not None:  # If this point must be pathable to another point
                ...
                for c in cansee:  # Check all nodes to see if a path can be made
                    if map.path_lookup(c, desired_connection) is not None:
                        print("found a point it can see that hit desired connection")
                        return c
        ...
        if point.x + search_size / 2 > ww and point.x - search_size / 2 < ww:
            print("SEARCH SIZE MAX HIT DOING DISTANCE ONLY")
            return nearest_node_by_distance(point, map, desired_connection)  # Give up on line of sight
...
def astar(start: Point, end: Point, map):
    ...
    while True:
        ...
        if time.time() - start_time > 10:  # More than 10 seconds, the path is too complicated
            return None
